...
class LotkaVolteraModel:
    def __init__(self, species_collection: SpeciesCollection, capacity: float = 1.0):
        """
    ...
    def growth_rates(self, population: np.ndarray) -> np.ndarray:
        """
        ...
        return (
            1
            - self.species_collection.interaction_matrix.dot(population) / self.capacity
        )

    def change(self, population: np.ndarray) -> np.ndarray:
        """
        ...
        population_coefficients = (
            population * self.species_collection.growth_rate_vector
        )
