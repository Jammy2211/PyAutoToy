...
class TestLotkaVoltera:
    def test_static(self, lotka_voltera_model):
        ...
        assert (lotka_voltera_model.step(
            population
        ) == population).all()
    ...
    def test_convergence(self, lotka_voltera_model):
        ...
        for _ in range(10):
            population = lotka_voltera_model.step(
                population
            )
            change = lotka_voltera_model.change(
                population
            )
