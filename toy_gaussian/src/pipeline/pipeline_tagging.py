...
def pipeline_tag_from_pipeline_settings(
    ...
    hyper_tag = hyper_tag_from_hyper_settings(
        hyper_gaussians=hyper_gaussians,
...
def hyper_gaussians_tag_from_hyper_gaussians(hyper_gaussians):
    """Generate a tag for if hyper_gaussians-gaussians are used in a hyper_gaussians pipeline to customize phase names.

    This changes the phase name 'pipeline_name__' as follows:

    fix_lens_light = False -> pipeline_name__
    fix_lens_light = True -> pipeline_name___hyper_gaussians
    """
    if not hyper_gaussians:
        return ""
    elif hyper_gaussians:
        return "_gaussians"


def hyper_image_sky_tag_from_hyper_image_sky(hyper_image_sky):
    """Generate a tag for if the sky-background is hyper as a hyper_gaussians-parameter in a hyper_gaussians pipeline to
...
def hyper_background_noise_tag_from_hyper_background_noise(hyper_background_noise):
    """Generate a tag for if the background noise is hyper as a hyper_gaussians-parameter in a hyper_gaussians pipeline to
    customize phase names.
...
def hyper_tag_from_hyper_settings(
    hyper_gaussians, hyper_image_sky, hyper_background_noise
):

    if not any([hyper_gaussians, hyper_image_sky, hyper_background_noise]):
        return ""

    hyper_gaussians_tag = hyper_gaussians_tag_from_hyper_gaussians(
        hyper_gaussians=hyper_gaussians
    )
