...
class TestPipelineNameTag:
    def test__pipeline_tag__mixture_of_values(self):
        pipeline_tag = al.pipeline_tagging.pipeline_tag_from_pipeline_settings(
            hyper_gaussians=True, hyper_image_sky=True, hyper_background_noise=True
        )

        assert pipeline_tag == "pipeline_tag__hyper_gaussians_bg_sky_bg_noise"
...
class TestHyperPipelineTaggers:
    def test__hyper_gaussians_tagger(self):
        tag = al.pipeline_tagging.hyper_gaussians_tag_from_hyper_gaussians(
            hyper_gaussians=False
        )
        assert tag == ""
        tag = al.pipeline_tagging.hyper_gaussians_tag_from_hyper_gaussians(
            hyper_gaussians=True
        )
        assert tag == "_gaussians"
    ...
    def test__tag_from_hyper_settings(self):
        tag = al.pipeline_tagging.hyper_tag_from_hyper_settings(
            hyper_gaussians=False, hyper_image_sky=False, hyper_background_noise=False
        )

        assert tag == ""

        tag = al.pipeline_tagging.hyper_tag_from_hyper_settings(
            hyper_gaussians=True, hyper_image_sky=False, hyper_background_noise=False
        )

        assert tag == "__hyper_gaussians"

        tag = al.pipeline_tagging.hyper_tag_from_hyper_settings(
            hyper_gaussians=False, hyper_image_sky=True, hyper_background_noise=True
        )
