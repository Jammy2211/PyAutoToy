...
class TestGaussian:
    def test__constructor_and_units(self):
        ...
        assert isinstance(gaussian.centre[0], g.dim.Length)
        assert isinstance(gaussian.centre[1], g.dim.Length)
        ...
        assert isinstance(gaussian.intensity, g.dim.Luminosity)
        assert gaussian.intensity.unit == "eps"

        assert gaussian.sigma == 0.1
        assert isinstance(gaussian.sigma, g.dim.Length)
        assert gaussian.sigma.unit_length == "arcsec"

        gaussian = g.SphericalGaussian(centre=(1.0, 2.0), intensity=1.0, sigma=0.1)
    ...
    def test__intensity_from_grid__change_geometry(self):
        ...
        gaussian_1 = g.EllipticalGaussian(
            centre=(3.0, 0.0), axis_ratio=0.5, phi=0.0, intensity=1.0, sigma=1.0
        )
    ...
    def test__spherical_and_elliptical_match(self):
        elliptical = g.EllipticalGaussian(
            axis_ratio=1.0, phi=0.0, intensity=3.0, sigma=2.0
        )
        spherical = g.SphericalGaussian(intensity=3.0, sigma=2.0)
