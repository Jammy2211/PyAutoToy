...
# In 'runners/gaussian_x1.py', we fitted a Gaussian image with the pipelines, 'pipeline__initialize__gaussian_x1' and
# 'pipeline__main__gaussian_x1'. The results of this fit are in the 'output_fixed/gaussian_x1' folder.
...
workspace_path = "/home/jammy/PycharmProjects/PyAuto/PyAutoToy/gaussian/workspace/"
...
#### Alternative, if changing the pipeline__meta is a pain, we could make it so that the aggregator only looks for whether the
...
pipeline__meta = "pipeline__initialize__x1_gaussian + pipeline__main__x1_gaussian"
...
optimizers = aggregator.optimizers_with(pipeline=pipeline__meta, phase=phase_name)
phases = aggregator.phases_with(pipeline=pipeline__meta, phase=phase_name)
...
most_probable_vector = list(
    map(lambda out: out.most_probable_vector, non_linear_outputs)
)
...
most_probable_instances = list(
    map(lambda out: out.most_probable_instance, non_linear_outputs)
)
...
results = aggregator.model_results(pipeline=pipeline__meta, phase=phase_name)
